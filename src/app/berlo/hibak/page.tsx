...
export default async function BerloiHibak() {
  ...
  return (
    <div className="grid gap-8">
      <section>
        <h1 className="text-2xl font-semibold tracking-tight">Hibabejelentés</h1>
        <p className="mt-1 text-stone-600 dark:text-stone-400">
          Amit ide beírsz, azt a bérbeadó a teendői között látja, határidővel. Az elhárítást
          te erősíted meg: amíg nem mondod, hogy rendben van, a bejelentés nyitva marad.
        </p>
      </section>

      <section>
        <h2 className="mb-3 text-lg font-semibold">Új bejelentés</h2>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold">A bérbeadó elérhetősége</h2>
          ...
          {elerhetosegek.map(({ jogviszony, berbeado }) => (
            ...
            >
              ...
              {berbeado && !berbeado.telefon ? (
                <p className="mt-1 text-stone-600 dark:text-stone-400">
                  Telefonszámot még nem adott meg. Veszélyhelyzetnél kérd el tőle, mert a
                  bejelentés magától nem csörög.
                </p>
              ) : null}
            </li>
          ))}
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold">
          Nyitott bejelentéseim{nyitottak.length > 0 ? ` (${nyitottak.length})` : ""}
        </h2>
        {nyitottak.length === 0 ? (
      ...
      {lezartak.length > 0 ? (
        <section>
          <h2 className="mb-3 text-lg font-semibold">Lezárt bejelentéseim</h2>
          <ul className="grid gap-3">
            {lezartak.map((hiba) => (
              <Hibakartya
                key={hiba.id}
